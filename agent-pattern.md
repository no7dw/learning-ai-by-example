...
| --- | --- | --- | --- | --- | --- | --- |
| Direct LLM / Single Agent | 一个模型或一个 Agent | 对话上下文 | 问答、简单分类、一次工具调用 | 低延迟、低成本、容易调试 | 复杂任务容易漏步骤 | 客服 FAQ、商品属性问答 |
| ReAct | Agent 在每一轮决定下一步 | 隐式消息历史 | 开放式搜索、工具调用、故障排查 | 自适应，遇到新信息能调整 | 路径不可预测，可能循环和浪费 token | 商品导购、跨平台商品研究 |
| Plan-and-Execute | Planner 生成计划，Executor 执行 | 计划 + step results | 目标明确、步骤较多、依赖可描述 | 全局视角强，进度清楚，可并行 | 计划可能过时，先规划会增加延迟 | 数据问答、月度经营分析 |
| Graph / Workflow | 代码或图状态决定下一节点 | 显式 state | 业务规则、审批、重试、可恢复流程 | 可测试、可追踪、边界清楚 | 分支多时维护成本高，灵活性较低 | 售后客服、退货退款 |
| Supervisor Multi-Agent | Manager 选择角色和交接 | 消息 + 共享任务状态 | 专业角色确实不同，任务可拆分 | 专业化、可并行、团队边界清晰 | 通信成本高，容易重复工作 | 自动投放、大促运营团队 |
...
订单客服是典型的 Single Agent 场景：Agent 负责识别订单、调用订单状态和物流查询工具，再把结果解释给用户。任务只有一个主要责任，虽然内部可能调用多个工具，但不需要商品、广告和库存 Agent 互相交接。
...
商品导购需要综合商品目录、详情、评价、价格、促销和区域库存。搜索结果会决定下一步调用哪个工具，因此路径不能完全提前写死。
...
数据问答和经营分析通常要组合订单、GMV、广告、库存、退货和品类数据。目标明确、步骤较多，且部分数据查询可以并行，因此适合先生成计划，再执行和重规划。
...
自动投放和大促运营同时涉及商品组合、库存、广告预算、渠道策略、客服话术和风险。每个领域有不同工具和质量标准，适合由 Manager 分派给专业 Agent，再由 Reviewer 汇总和检查冲突。
...
这是一个电商大促运营场景：系统需要找出高潜商品、检查库存、估算广告预算、生成客服话术，并形成最终可执行方案。
